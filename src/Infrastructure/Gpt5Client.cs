...
public sealed class Gpt5Client
{
    ...
    public Gpt5Client(IConfiguration cfg, ILogger<Gpt5Client> logger, IHttpClientFactory factory)
    {
        ...
        _azureApiKey = _cfg["AzureOpenAI:ApiKey"];

        if (_useAzure)
        {
            if (string.IsNullOrWhiteSpace(_azureApiKey))
            {
                _logger.LogWarning("AzureOpenAI:ApiKey nao definido (use User Secrets).");
            }
        ...
        else
        {
            ...
            if (string.IsNullOrWhiteSpace(apiKey))
                _logger.LogWarning("OPENAI_API_KEY nao definido. Chamadas OpenAI falharao sem chave.");
    ...
    private string BuildAzureUrl()
    {
        ...
        if (string.IsNullOrWhiteSpace(_azureEndpoint) ||
            string.IsNullOrWhiteSpace(_azureDeployment) ||
            string.IsNullOrWhiteSpace(_azureApiVersion))
            throw new InvalidOperationException("Configuracao AzureOpenAI incompleta (Endpoint/Deployment/ApiVersion).");
    ...
    public async Task<(string content, int tokensIn, int tokensOut)> ChatJsonAsync(
        string systemPrompt,
        string userPrompt,
        ...
        CancellationToken ct = default)
    {
        ...
        if (_useAzure)
        {
            var format = responseFormat == ResponseFormat.QuestionsToon
                ? ToonCodec.AzureQuestionSchema
        ...
        {
            _logger.LogWarning("Prompt length {Len} acima de {Max}; reduza entradas ou max prompt length.", json.Length, _maxPromptLength);
        }
        ...
        if (_useAzure && responseFormat == ResponseFormat.QuestionsToon)
        {
